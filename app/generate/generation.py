...
class NetworkGenerator:
    ...
    def __init__(self, params):
        ...
        self.stage_colors = ['paleturquoise', 'lightskyblue', 'royalblue',
                             'blue']
        min_al = 3
        ...
        # }
        # yaml does not support objects with tuples as keys.
        # This is a hacky workaround.
        self.from_to = {(int(k[1]), int(k[3])): v for k, v in
                        from_to_str.items()}
    ...
    def create_base_network_object(self, pos_map, starting_node=0, *, nodes,
                                   links, network_id, n_steps, **kwargs):
        return {
    ...
    def get_max_reward(self, network):
        ...
        Q = calculate_q_value(edges, rewards, n_steps=self.n_steps,
                              n_nodes=len(network['nodes']), gamma=0)
        # get trace corresponding to q values
        edge_trace, node_trace = calculate_trace(Q, edges,
                                                 starting_node=network[
                                                     'starting_node'])
        reward_trace = rewards[edge_trace]
        max_reward = reward_trace.sum()

        max_reward2 = th.where(network['starting_node'] == edges[:, 0], Q[0],
                               th.tensor(-10000.)).max()
